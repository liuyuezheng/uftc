<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:64:"D:\park\public/../application/admin\view\stopcar\list_index.html";i:1543284981;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
	<form name="form" method='post' action="<?php echo url('admin/stopcar/listIndex'); ?>">
		<div style="margin-left:30px;margin-bottom:18px;">
			<div data-toggle="distpicker">
				<div data-toggle="distpicker">
				  ...
				  <div class="form-group">
			车牌号: <input type="text" class="input-text" style="width:150px;margin-right:10px;" placeholder="请输入内容" value="<?php echo $shuju['data']['plate']; ?>" name="plate" >
		...
		<?php else: ?>
		<div class="form-group">
			车牌号: <input type="text" class="input-text" style="width:150px;margin-left:20px;" placeholder="请输入内容" value="<?php echo $shuju['data']['plate']; ?>" name="plate" >
			<input type="submit" class="btn btn-success radius" value="搜索">
		</div>
		<?php endif; ?>
	</form>
	 <span cass="l" style="margin-left:30px;"> <a href="javascript:;" onclick="member_add('添加出入登记','<?php echo url('admin/stopcar/add'); ?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加出入登记</a></span>
	<!--  <span cass="l" style="margin-left:10px;"> <a href="javascript:;" onclick="member_add('开启道闸','<?php echo url('admin/stopcar/startcha'); ?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 开启道闸</a></span> -->
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<?php else: if(is_array($shuju['page']) || $shuju['page'] instanceof \think\Collection || $shuju['page'] instanceof \think\Paginator): $i = 0; $__LIST__ = $shuju['page'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
				...
				<td><?php echo round($val['timelong']/3600,2); ?></td>
				<td><?php echo date('Y-m-d H:i:s',$val['in_time']); ?></td>
				<td><?php echo date('Y-m-d H:i:s',$val['out_time']); ?></td>
				<td><?php echo $val['in_photo']; ?></td>
				<td><?php echo $val['out_photo']; ?></td>
			</tr>
