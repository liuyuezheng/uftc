<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:55:"D:\park\public/../application/admin\view\car\index.html";i:1535704667;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
	<form name="form" method='post' action="<?php echo url('admin/car/index'); ?>">
		...
		<div class="form-group" style="margin-left:30px;">
			...
			联系方式: <input type="text" class="input-text" style="width:150px;" placeholder="请输入内容" value="<?php echo $shuju['data']['phone']; ?>" name="phone" >
			状态: <select name='type' style='width:150px;margin-right:10px;' class="input-text">
					<option value='0'>--请选择--</option>
					<option value='1' <?php if($shuju['data']['type'] == '1'): ?>selected<?php endif; ?>>审核通过</option>
					<option value='2' <?php if($shuju['data']['type'] == '2'): ?>selected<?php endif; ?>>审核不通过</option>
					<option value='3' <?php if($shuju['data']['type'] == '3'): ?>selected<?php endif; ?>>审核中</option>
				 </select>
			<input type="submit" class="btn btn-success radius" value="搜索">
			<?php if($shuju['data']['mg_name'] == 'admin'): ?>
			<span cass="l" style="margin-left:10px;"> <a href="javascript:;" onclick="member_add('导入','<?php echo url('admin/car/enterport'); ?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 导入</a></span>
			<?php endif; ?>
		</div>
	</form>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				<th width="100">车牌号</th>
				<th width="100">车辆所属用户</th>
				<th width="100">车辆品牌</th>
				<th width="100">车辆型号</th>
				<th width="100">车辆排量</th>
		...
		<tbody>
			...
			<?php else: if(is_array($shuju['page']) || $shuju['page'] instanceof \think\Collection || $shuju['page'] instanceof \think\Paginator): $i = 0; $__LIST__ = $shuju['page'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
				...
				<td><?php echo $val['displacement']; ?></td>
				<td><?php echo $val['phone']; ?></td>
				<td>
					<?php if($val['type'] == '1'): ?>审核通过<?php endif; if($val['type'] == '2'): ?>审核不通过<?php endif; if($val['type'] == '3'): ?>审核中<?php endif; ?>
				</td>
				<td class="td-status"><a title="审核" href="javascript:;" onclick="member_edit('审核','<?php echo url('admin/car/verify',['car_id'=>$val['car_id']]); ?>','4','','510')" style="text-decoration:none"><span class="label label-success radius">审核</span></a></td>
				<!-- <td class="td-manage">
					...
					<a title="删除" href="javascript:;" onclick="member_del(this,<?php echo $val['uid']; ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
				</td> -->
			</tr>
