<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:58:"D:\park\public/../application/admin\view\device\index.html";i:1535774411;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
	<form name="form" method='post' action="<?php echo url('admin/device/index'); ?>">
		<div style="margin-left:30px;margin-bottom:18px;">
			<div data-toggle="distpicker">
				...
				<?php if($shuju['data']['mg_name'] == 'admin'): ?>
				  ...
				  <div class="form-group" >
			序列号: <input type="text" class="input-text" style="width:150px;" placeholder="请输入内容" value="<?php echo $shuju['data']['serialno']; ?>" name="serialno" >
		...
		<div class="form-group" style="margin-left:20px;">
			车位编号: <input type="text" class="input-text" style="width:150px;" placeholder="请输入内容" value="<?php echo $shuju['data']['serialno']; ?>" name="serialno" >
			<input type="submit" class="btn btn-success radius" value="搜索">
		</div>
		<?php endif; ?>
		
	</form>
	 <span cass="l" style="margin-left:30px;"> <a href="javascript:;" onclick="member_add('添加设备','<?php echo url('admin/device/add'); ?>','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加设备</a></span>
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		<thead>
			<tr class="text-c">
				...
				<th width="100">类型</th>
				<th width="100">道闸端口</th>
				<th width="100">开启道闸</th>
				<th width="100">操作</th>
			</tr>
		</thead>
		<tbody>
			...
			<?php else: if(is_array($shuju['page']) || $shuju['page'] instanceof \think\Collection || $shuju['page'] instanceof \think\Paginator): $i = 0; $__LIST__ = $shuju['page'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
				...
				<td class="td-status"><a title="开启" href="javascript:;" onclick="startcha(this,<?php echo $val['id']; ?>)" style="text-decoration:none"><span class="label label-success radius">开启</span></a></td>
				<td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','<?php echo url('admin/device/edit',['id'=>$val['id']]); ?>','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
	function startcha(obj,id){
		$.ajax({
			type: 'POST',
			url: '<?php echo url("admin/device/startcha"); ?>',
			dataType: 'json',
			data:{id:id},
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo url("admin/device/delete"); ?>',
			dataType: 'json',
			data:{id:id},
			success: function(data){
