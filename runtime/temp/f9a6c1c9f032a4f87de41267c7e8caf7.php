<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:60:"D:\park\public/../application/admin\view\carposs\sindex.html";i:1542957503;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
...
<div class="page-container">
	<form name="form" method='post' action="<?php echo url('admin/carposs/sindex'); ?>">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort">
		...
		<tbody>
			...
			<?php else: if(is_array($shuju['page']) || $shuju['page'] instanceof \think\Collection || $shuju['page'] instanceof \think\Paginator): $i = 0; $__LIST__ = $shuju['page'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?>
				...
				<td>
					<?php if($val['style'] == '1'): ?>审核通过<?php endif; if($val['style'] == '2'): ?>审核不通过<?php endif; if($val['style'] == '3'): ?>等待审核<?php endif; ?>
				</td>
					<td><?php echo $val['create_time']; ?></td>
					<td><?php echo $val['update_time']; ?></td>
					<!--<td><?php echo $val['plate']; ?></td>-->
				<td class="td-status"><a title="审核" href="javascript:;" onclick="member_edit('审核','<?php echo url('admin/carposs/verify',['id'=>$val['id']]); ?>','4','','510')" style="text-decoration:none"><span class="label label-success radius">审核</span></a></td>
				<!-- <td class="td-manage">
					<a title="编辑" href="javascript:;" onclick="member_edit('编辑','<?php echo url('admin/carpos/sedit',['id'=>$val['id']]); ?>','4','','510')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php echo url("admin/carpos/shanchu"); ?>',
			dataType: 'json',
			data:{id:id},
			success: function(data){
