...
class RentController extends BaseController
{
    ...
    {
        $param = request()->only(['orderId','uid','type']);
        $this ->ParkListValidate->scene('pay', ['orderId','uid','type']);
        ...
        if ($param['type'] == 1) {
            $response = $this -> PayService -> aliPay($param['orderId']);
        } else {
            $response = $this -> PayService -> wxPay($param['orderId']);
        }

        if (false !== $response) {
            return success(1000, ['response'=>$response], '获取数据成功');
        }
    }
    ...
    public function alipayCallBack()
    {
        ...
        $aop = new \AopClient;
        $aop->alipayrsaPublicKey = '';
        $flag = $aop->rsaCheckV1($_POST, null, "RSA2");
        if ($flag) {
            if ($param['trade_status'] === 'TRADE_SUCCESS') {
                if ($this->getType($orderId) === 'pay') {
                    $data = $this->RentModel->getOrder($orderId);
                    $result = $this->RentModel->payCallBack($data);
                    echo $result;
                } else {
                    $rechargesModel = new Recharges();
                    $data = $rechargesModel->getOrder($orderId);
                    $result = $rechargesModel->rechargesSuccess($data);
                    echo $result;
                }
            }
        }
    ...
    private function getType($orderId)
    {
        return strpos($orderId, 'c') !== false ? 'recharges': 'pay';
    }
}
