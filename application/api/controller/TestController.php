...
class TestController extends BaseController
{
    public function test2(){
        $imgStr = file_get_contents('base64.txt');
        $res = base64_image_content($imgStr);
        $param = [];
        var_dump($res);
        $param['logo'] = implode(',,,', array_slice($res, 0, 2));
        $param['dlogo'] = implode(',,,', array_slice($res, 2, 2));
        vv($param);
    }
...
//        ->select();
        $info=DB::table('pk_carpos')->alias('c')
            ...
            ->field('c.id,c.number,c.types,m.name,m.address,m.area,c.end_time1,m.longitude,m.latitude')
            ->select();
        foreach($info as $k=>$v){
...
//        $data=array_merge($personal,$shuju,$shuju1,$info);
        $data=array_merge($personal,$shuju,$info);
        if(!empty($data)){
