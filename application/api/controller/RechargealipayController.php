<?php

namespace app\api\controller;

use think\Controller;
use think\Db;

class RechargealipayController extends Controller
{
    /**
     *充值支付宝支付
     */
    public function alipay_pay($order_num) {
      
        $res = Db::table('pk_recharges')->where(['order_id'=>$order_num])->find();
       
        if(empty($res)){
            return json_encode(['code'=>1001,'msg'=>'订单错误']);
        }
        $name = '充值';
        $order_num = $res['order_id'];
        //获取价格
        $price = $res['account'];
        $price = 0.01;
        
        vendor('alipay.aop.AopClient');
        $aop = new \AopClient;
        $aop->gatewayUrl = "https://openapi.alipay.com/gateway.do";
        $aop->appId = "2017091808802313";
        $aop->rsaPrivateKey = '';
        $aop->format = "json";
        $aop->charset = "UTF-8";
        $aop->signType = "RSA2";
        $aop->alipayrsaPublicKey = '';
        //实例化具体API对应的request类,类名称和接口名称对应,当前调用接口名称：alipay.trade.app.pay
        vendor('alipay.aop.request.AlipayTradeAppPayRequest');
        $request = new \AlipayTradeAppPayRequest();
        //SDK已经封装掉了公共参数，这里只需要传入业务参数
        $bizcontent = "{\"body\":\"\","
            . "\"subject\": \"" . $name . "\","
            . "\"out_trade_no\": \"" . $order_num . "\","
            . "\"timeout_express\": \"30m\","
            . "\"total_amount\": \"".$price."\","
            . "\"product_code\":\"QUICK_MSECURITY_PAY\""
            . "}";
        $notify_url = 'http://www.park.com/api/Rechargealipay/alipay_callback';
        $request->setNotifyUrl($notify_url);
        $request->setBizContent($bizcontent);
        //这里和普通的接口调用不同，使用的是sdkExecute
        $response = $aop->sdkExecute($request);//htmlspecialchars是为了输出到页面时防止被浏览器将关键参数html转义，实际打印到日志以及http传输不会有这个问题
        return json_encode(['code'=>1000,'data'=>$response,'msg' =>'数据获取成功']);

        // return return_json_format_return($data);
    }
    /**
     *支付宝支付成功回调
     */
    public function alipay_callback() {
        vendor('alipay.aop.AopClient');
        $aop = new \AopClient;
        $aop->alipayrsaPublicKey = '';
        $flag = $aop->rsaCheckV1($_POST, NULL, "RSA2");
        $flag_str = json_encode($flag);
        if ($flag) {
            if(!empty($_POST['out_trade_no'])){

                 file_put_contents('3.txt',$_POST['out_trade_no']);
                //更改状态
               Db::startTrans();
                try{
                    $order_res = Db::table('pk_recharges')->where(['order_id'=>$_POST['out_trade_no']])->find();
                    if(empty($order_res)){
                        Db::rollback();
                        return 2;
                    }
                    $data = [
                        'status'=>1
                    ];
                    $res = Db::table('pk_recharges')->where(['order_id'=>$_POST['out_trade_no']])->update($data);
                    if(empty($res)){
                        Db::rollback();
                        return 2;
                    }
                    $where2 = [
                        'uid'=>$order_res['uid'],
                    ];
                    $u_res = Db::table('pk_user')->where($where2)->setInc('moneybag',$order_res['account']);
                    if(empty($ma_res)){
                        Db::rollback();
                        return 2;
                    }
                    
                    Db::commit();
                } catch (\Exception $e) {
                    // 回滚事务
                    Db::rollback();
                }

            }
        }
    }
}

