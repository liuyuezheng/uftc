...
class IndexController extends BaseController
{
    public function index()
    {
        ...
        $info=Db::table('pk_manager')->where($where)->field('longitude,latitude,mg_id,total,area,address,leftpos')->select();
        if(!empty($info)){
            foreach($info as $k=>$v){
    ...
    public function indexs(){
        $uid=input('uid');
        if(!empty($uid)){
            $shuju=Db::table('pk_stopcarrecord')->alias('s')
            ->join('__MANAGER__ m','m.mg_id=s.mg_id')
            ->where(['s.uid'=>$uid])
            ->order('s.id desc')
            ->limit(1)
            ->field('m.name,s.number,s.mg_id')
            ->find();
            if(empty($shuju)){
               $shuju=[];
            }
...
from pk_manager where city='".$city."' order by leftpos desc limit " .$start.",".$num;
    ...
    public function detail(){
        ...
        if(!empty($data)){
            return json_encode(['code'=>1000,'data'=>$data,'shuju'=>$shuju,'msg'=>'首页停车场详细信息获取成功']);
        }else{
    ...
    public function bindplate(){
        ...
        if(!empty($res)){
            ...
            return josn_encode(['code'=>1001,'msg'=>'车牌绑定失败']);
        }
    }
