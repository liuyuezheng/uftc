...
class RechargewxpayController extends Controller
{
    public function wx_pay($order_id){
        $res = Db::table('pk_recharges')->where(['order_id' => $order_id])->find();
...
//        $input->SetAttach($type);
        ...
        $notify_url = 'http://www.park.com/api/Rechargewxpay/wxpay_callback';
    ...
    public function wxpay_callback(){
        ...
        if ($result['result_code'] === 'SUCCESS' && $result['mch_id'] === \WxPayConfig::MCHID && $result['appid'] === \WxPayConfig::APPID) {
            ...
            if ($sign === $result['sign']) {
                 file_put_contents('33.txt', $result['out_trade_no']);
                 //更改状态
               Db::startTrans();
                try{
                    $order_res = Db::table('pk_recharges')->where(['order_id'=>$_POST['out_trade_no']])->find();
                    if(empty($order_res)){
                        Db::rollback();
                        return 2;
                    }
                    ...
                    $res = Db::table('pk_recharges')->where(['order_id'=>$_POST['out_trade_no']])->update($data);
                    if(empty($res)){
                        Db::rollback();
                        return 2;
                    }
                    ...
                    $u_res = Db::table('pk_user')->where($where2)->setInc('moneybag',$order_res['account']);
                    if(empty($ma_res)){
                        Db::rollback();
                        return 2;
                    }
