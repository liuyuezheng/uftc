...
//        $whererent=['c.uuid'=>['ne']];
        ...
        $personal2=DB::table('pk_carpos')->alias('c')
            ...
            }) ->whereOr(function ($query) use ($whererent3) {
                $query->where($whererent3);
...
//        }
        $person=array_merge($personal,$personal2);
        //出租车位
        $wherezu3=[
            'c.uuid'=>['=',$uid],
            'c.uuuid'=>['eq',0],
        ...
        $info=DB::table('pk_carpos')->alias('c')
                ->join('__MANAGER__ m','c.mg_id=m.mg_id')
                ->where($wherezu3)
                 ->whereOr(function ($query) use ($whereg4) {
                $query->where($whereg4);
                })
            ->order('c.update_time desc')
                ->field('c.id,c.number,c.types,m.name,m.address,m.area,c.start_time,c.end_time,c.start_time1,c.end_time1,m.longitude,m.latitude')
        ->select();
        foreach($info as $key=>$val){
            if($val['types']==3){
                $info[$key]['start_time']=$val['start_time1'];
                $info[$key]['end_time']=$val['end_time1'];
                unset($info[$key]['end_time1']);
                unset($info[$key]['start_time1']);
            }else{
                unset($info[$key]['end_time1']);
                unset($info[$key]['start_time1']);
            }
        }
//        $a=json_encode(['info'=>$info,'per'=>$person]);
//        $b = json_decode($a)->info;
//        $b1 = json_decode($a)->per;
//        $c = array_merge($info,$person);
//        dump($c);
//        die();
//        $list=array_merge($info);
//        $list=$info;
//        $list=$info;
        $data=array_merge($info,$person);
//        $lists[]=$info;
//        $data=$person+$info;
