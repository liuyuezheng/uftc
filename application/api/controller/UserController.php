...
//        }
    ...
    public function recharge(){
        ...
        unset($param['pay']);
        $param['status'] = 2;
        $param['order_id'] = 'c'.date('YmdHis') . str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);

        $insertRes = Db::table('pk_recharges')->insert($param);
        $response = $this->PayService->aliPay($param['order_id']);
        if (false !== $response) {
...
//        }
    ...
    public function tieup(){
        ...
        $exists = DB::table('pk_user')->where(['phone'=>$param['phone']])->find();
