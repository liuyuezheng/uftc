    ...
    public function welcome() 
    {
        ...
        if($mg_name == 'admin'){
            ...
            foreach($data as $k=>$v){
                ...
                $data[$k]['money']=Db::table('pk_revenue')->where(['type'=>1,'mg_id'=>$v['mg_id']])->sum('account');
            }
            $this->assign(['data'=>$data,'manager_number'=>$manager_number,'mg_name'=>$mg_name]);
            return $this->fetch();
        }else{
            //用户数量
            ...
            $data['money']=Db::table('pk_revenue')->where(['type'=>1,'mg_id'=>$mg_id])->sum('account');
