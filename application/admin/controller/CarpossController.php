...
//            $data['logo']=input('logo');
            ...
            $manager=input('mg_id');
            $exists = Db::table('pk_carpos')->where(['mg_id'=>$data['mg_id'],'number'=>$data['number'],'owner'=>2])->where('uid','neq',$data['uid'])->find();
//            return json_encode($exists);
            if(!empty($exists) || $exists!=null){
                return ['status'=>'failure','errorinfo'=>"该车位已有人"];
            }else{
                ...
                $excel2 = Db::table('pk_carpos')->where(['mg_id'=>$data['mg_id'],'number'=>$data['number'],'style'=>1,'owner'=>1])->find();
...
//                    return json_encode($excel);
                    ...
                    if(empty($logo)){
                        ...
                        if(!empty($excel2) || $excel2!=null){
                            $res=$carpos->where('id',$excel2['id'])->update(['uid'=>$data['uid'],'owner'=>2,'types'=>1]);
                        }else{
...
//                $data['logo']=input('logo');
            ...
            ->where(['c.id'=>$id])
            ->field('c.*,u.username,m.name,m.province,m.city,m.area,c.mg_id,c.sensor,c.charge,c.charge1,c.types')->find();
...
//                    'endtime'=>$list['endtime'],
            ...
            $id=$request->param('id');
         $logo=DB::table('pk_carpos')->where(['id'=>$id])->field('logo2,uid,mg_id,number')->find();
            $arr=explode(",,,.",$logo['logo2']);
            $arr[0]=substr($arr[0], 1);
//            return json_encode($arr);
            $this->assign(['id'=>$id,'logo'=>$arr,'uid'=>$logo['uid'],'mg_id'=>$logo['mg_id'],'number'=>$logo['number']]);
            return $this->fetch();
        }
    }
