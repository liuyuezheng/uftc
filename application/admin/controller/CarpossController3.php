...
//        $data['mg_id']=Session::get('mg_id');
        ...
        $data['mg_id']=$request->param('mg_id/d');
...
//                    return json_encode($excel);
                    ...
                    if(empty($logo)){
                        ...
                        if(!empty($excel2) || $excel2!=null){
                            $res=$carpos->where('id',$excel2['id'])->update(['uid'=>$data['uid'],'owner'=>2]);
                        }else{
...
//                    'endtime'=>$list['endtime'],
            ...
            $this->assign(['id'=>$id,'logo'=>getImage($logo['logo2']),'uid'=>$logo['uid'],'mg_id'=>$logo['mg_id'],'number'=>$logo['number']]);
            return $this->fetch();
        }
    }
